...
const AnalyticsScreen: React.FC = () => {
  ...
  const { revenueAnalytics, isLoading, error } = useSelector((state: RootState) => state.analytics);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <Text style={styles.summaryValue}>
              {revenueAnalytics?.totalRevenue ? 
                new Intl.NumberFormat('en-US', {
                  style: 'currency',
                  currency: 'USD',
                }).format(revenueAnalytics.totalRevenue) : '$0.00'}
            </Text>
          </View>
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>
              {revenueAnalytics?.averageRevenue ? 
                new Intl.NumberFormat('en-US', {
                  style: 'currency',
                  currency: 'USD',
                }).format(revenueAnalytics.averageRevenue) : '$0.00'}
            </Text>
          </View>
        </View>
        <View style={styles.summaryRow}>
          <View style={styles.summaryItem}>
            ...
            <Text style={[
              styles.summaryValue,
              revenueAnalytics?.growth && revenueAnalytics.growth > 0 ? styles.positiveValue : 
              revenueAnalytics?.growth && revenueAnalytics.growth < 0 ? styles.negativeValue : {}
            ]}>
              {revenueAnalytics?.growth ? `${revenueAnalytics.growth > 0 ? '+' : ''}${revenueAnalytics.growth}%` : '0%'}
            </Text>
          </View>
          <View style={styles.summaryItem}>
      ...
      <Card style={styles.detailsCard}>
        ...
        {revenueAnalytics?.breakdown && Object.entries(revenueAnalytics.breakdown).map(([category, value]) => (
      ...
      <Card style={styles.trendsCard}>
        ...
        <Text style={styles.trendsNote}>
          Note: In a production app, this would display an actual chart using a library like react-native-chart-kit or Victory Native.
        </Text>
      </Card>
      ...
      <Card style={styles.insightsCard}>
        ...
        {revenueAnalytics?.insights ? (
          ...
          <Text style={styles.noDataText}>No insights available for the selected period.</Text>
        )}
      </Card>
    </ScrollView>
  );
