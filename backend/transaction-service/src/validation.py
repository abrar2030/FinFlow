...
class TransactionValidator:
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_levels": {
                ...
                "high_threshold": 0.8,
                "critical_threshold": 0.9,  # Added for clarity
            },
            "aml_checks": {
                "enabled": True,
    ...
    def _validate_amount(
        ...
        # Check if amount is positive (Pydantic handles this, but kept for custom logic)
    ...
    def _validate_aml_cft(
        ...
        # NOTE: The original code assumes 'country_code' is in context, which is a good practice.
    ...
    def _detect_fraud_patterns(
        ...
        if self.config["fraud_detection"]["check_transaction_patterns"]:
            ...
            # NOTE: The original code was passing context.get("is_unusual_time", False) to _calculate_pattern_risk,
            # but the check is done here. I will update the context enrichment in validate_transaction
            # to include these flags if they were calculated elsewhere, but for now, I'll assume
            # the helper method is meant to perform the check.
            if self._is_unusual_transaction_time(transaction, context):
                ...
                # Add to context for risk scoring
    ...
    def _determine_risk_level(self, risk_score: float) -> RiskLevel:
        ...
        # NOTE: The original code was missing a check for CRITICAL.
        # I've added 'critical_threshold' to _default_config and use it here.
    ...
    # Helper methods for validation (Mocks)
    ...
    def _calculate_amount_risk(self, transaction: TransactionRequest) -> float:
        ...
        if transaction.amount <= high_value:
            ...
            # NOTE: The original code had a division by zero risk if high_value == very_high_value.
            # I'll assume the config ensures very_high_value > high_value.
            # The calculation is correct for linear scaling.
            ...
            # For amounts > very_high_value, the risk should increase further.
            # The original formula is a bit arbitrary: 0.5 + 0.5 * min(1.0, (amount - very_high_value) / very_high_value)
            # This means the max risk is 1.0, reached when amount = 2 * very_high_value.
            # I will keep the logic as it is a mock, but ensure no division by zero.
    ...
    def _calculate_velocity_risk(self, transaction: TransactionRequest) -> float:
        ...
        # The original logic was max(minute_risk, hour_risk * 0.8), which is fine for a mock.
    ...
    def _calculate_pattern_risk(
        ...
        # NOTE: The flags 'is_unusual_time' and 'is_unusual_location' are set in _detect_fraud_patterns
        # and passed through the context.
        ...
        return min(1.0, risk)  # Ensure risk score component does not exceed 1.0
...
class BatchTransactionValidator:
    ...
    def validate_batch(
    ...
    ]:  # FIX: The return type was List[ValidationResult] but the logic returns a Dict
        """
        ...
        for transaction in transactions:
            ...
            # NOTE: The context enrichment should be done on a copy of the base context
            # to avoid side effects across transactions in the batch.
...
# Example usage (added for completeness and testing)
if __name__ == "__main__":
    ...
    # 2. High-risk transaction (High amount, unusual location)
    high_risk_tx = TransactionRequest(
        transaction_id="TX1002",
        source_account_id="ACC001",
        destination_account_id="ACC003",
        amount=60000.00,  # Exceeds high_value (10k) and very_high_value (50k)
        currency="USD",
        transaction_type=TransactionType.TRANSFER,
        timestamp=datetime.now().isoformat(),
    ...
    high_risk_context = {
        ...
        "ip_address": "1.1.1.1",
        "location": "Kabul",
    }  # High risk country, unusual location
    ...
    # 3. Invalid transaction (Sanctioned country)
