...
from .tax_calculation_engine import (
    TaxCalculationEngine, Transaction, TaxProfile, 
    TaxType, CalculationMethod, create_sample_data
)
from .tax_rule_management import TaxRuleManager, SAMPLE_TAX_RULES
...
def calculate_tax():
    ...
    try:
        ...
        required_fields = ['transaction_id', 'amount', 'transaction_type', 
                          'origin_jurisdiction', 'destination_jurisdiction',
...
def create_tax_rule():
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['rule_id', 'jurisdiction', 'tax_type', 'effective_date', 'rate', 'calculation_method']
        for field in required_fields:
            if field not in data:
                return jsonify({'error': f'Missing required field: {field}'}), 400
