...
from tax_automation.tax_calculation_engine import (
    TaxCalculationEngine, TaxRule, TaxProfile, Transaction,
    TaxType, CalculationMethod, create_sample_data
)
from tax_automation.tax_rule_management import TaxRuleManager
from tax_automation.international_compliance import (
    InternationalComplianceManager, ComplianceCheckType, 
...
class TestInternationalCompliance(unittest.TestCase):
    ...
    def test_kyc_check(self):
        ...
        # Perform KYC check
        kyc_result = self.compliance_manager.kyc_service.perform_kyc_check('test_entity_002')
        
        self.assertEqual(kyc_result.entity_id, 'test_entity_002')
        self.assertEqual(kyc_result.check_type, ComplianceCheckType.KYC)
        self.assertIn(kyc_result.status, [ComplianceStatus.PASSED, ComplianceStatus.REQUIRES_REVIEW])
    ...
    def test_fatca_check(self):
        ...
        # Perform FATCA check
        fatca_result = self.compliance_manager.fatca_service.check_us_person_status('test_entity_003')
    ...
    def test_aml_transaction_monitoring(self):
        ...
        monitoring_result = self.compliance_manager.aml_service.monitor_transaction(transaction_data)
    ...
    def test_comprehensive_compliance_check(self):
        ...
        results = self.compliance_manager.perform_comprehensive_compliance_check('test_entity_005')
...
class TestIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_compliance_db = tempfile.NamedTemporaryFile(delete=False, suffix='.db')
        self.temp_compliance_db.close()
        
        self.tax_manager = TaxRuleManager(self.temp_tax_db.name)
        self.compliance_manager = InternationalComplianceManager(self.temp_compliance_db.name)
    ...
    def test_end_to_end_workflow(self):
        ...
        compliance_profile = self.compliance_manager.create_entity_profile(entity_profile_data)
        ...
        compliance_results = self.compliance_manager.perform_comprehensive_compliance_check('integration_entity_001')
        ...
        aml_result = self.compliance_manager.aml_service.monitor_transaction(aml_transaction_data)
