...
class TaxRuleEngine:
    ...
    @staticmethod
    def evaluate_conditions(conditions: Dict[str, Any], transaction: Transaction, 
                          tax_profile: TaxProfile) -> bool:
...
class TaxCalculationEngine:
    ...
    def get_applicable_rules(self, transaction: Transaction, 
                           tax_profile: TaxProfile) -> List[TaxRule]:
        ...
        for rule in self.tax_rules:
            ...
            if (rule.jurisdiction != transaction.origin_jurisdiction and 
                rule.jurisdiction != transaction.destination_jurisdiction and
                rule.jurisdiction != tax_profile.tax_residency):
                continue
            
            # Evaluate conditions
            if self.rule_engine.evaluate_conditions(rule.conditions, transaction, tax_profile):
    ...
    def calculate_taxes(self, transaction: Transaction) -> TaxCalculationResult:
        ...
        try:
            ...
            if not payer_profile:
                result.errors.append(f"Tax profile not found for payer: {transaction.payer_entity_id}")
                return result
            
            if not payee_profile:
                result.warnings.append(f"Tax profile not found for payee: {transaction.payee_entity_id}")
            ...
            logger.info(f"Calculated taxes for transaction {transaction.transaction_id}: "
                       f"Total tax amount: {result.total_tax_amount}")
            
        except Exception as e:
            logger.error(f"Error calculating taxes for transaction {transaction.transaction_id}: {e}")
