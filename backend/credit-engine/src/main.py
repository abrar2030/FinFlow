...
async def score_credit(
    ...
    logger.info(f"Credit score request for user {current_user['sub']}: {json.dumps(input_data.dict())}")
    ...
    try:
        ...
        logger.info(f"Credit score response for user {current_user['sub']}: {json.dumps(response.dict())}")
...
async def get_loan_offers(
    ...
    # Log the request (audit logging)
    logger.info(f"Loan offers request for user {current_user['sub']} with score {score}")
    ...
    if score >= 0.8:
        ...
        offers = [
            LoanOffer(amount=50000, interest_rate=4.5, term_months=60, monthly_payment=932.78),
            LoanOffer(amount=25000, interest_rate=4.2, term_months=36, monthly_payment=738.99),
            LoanOffer(amount=10000, interest_rate=3.9, term_months=24, monthly_payment=434.65)
        ]
    elif score >= 0.6:
        # Medium risk
        offers = [
            LoanOffer(amount=25000, interest_rate=6.5, term_months=60, monthly_payment=488.75),
            LoanOffer(amount=10000, interest_rate=6.2, term_months=36, monthly_payment=304.98),
            LoanOffer(amount=5000, interest_rate=5.9, term_months=24, monthly_payment=221.24)
        ]
    else:
        # High risk - limited offers
        offers = [
            LoanOffer(amount=5000, interest_rate=9.9, term_months=36, monthly_payment=161.42),
            LoanOffer(amount=2500, interest_rate=8.9, term_months=24, monthly_payment=114.58)
        ]
    ...
    # Log the response (audit logging)
    logger.info(f"Loan offers response for user {current_user['sub']}: {len(offers)} offers provided")
...
async def export_user_data(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "credit_scores": [
            {"score": 0.75, "risk_category": "MEDIUM_RISK", "timestamp": "2025-01-15T14:30:00"},
            {"score": 0.82, "risk_category": "LOW_RISK", "timestamp": "2025-04-20T09:15:00"}
        ],
        "loan_applications": [
