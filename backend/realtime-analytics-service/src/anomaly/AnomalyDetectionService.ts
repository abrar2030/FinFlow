...
export class AnomalyDetectionService extends EventEmitter {
  ...
  private async initializeStatisticalModels(): Promise<void> {
    const modelTypes = ['transaction_amount', 'transaction_frequency', 'payment_velocity', 'user_behavior'];
    ...
    for (const modelType of modelTypes) {
      const threshold: StatisticalThreshold = {
        ...
        mean: 0,
        standardDeviation: 0,
        zScoreThreshold: config.anomalyDetection.models.statisticalThreshold.zScoreThreshold,
        movingAverageWindow: config.anomalyDetection.models.statisticalThreshold.movingAverageWindow,
        dataPoints: [],
  ...
  private async initializeIsolationForestModels(): Promise<void> {
    const modelTypes = ['transaction_patterns', 'payment_patterns', 'user_patterns'];
    
    for (const modelType of modelTypes) {
      const model: IsolationForestModel = {
        modelType,
        contamination: config.anomalyDetection.models.isolationForest.contamination,
  ...
  private getModelFeatures(modelType: string): string[] {
    switch (modelType) {
      case 'transaction_patterns':
        return ['amount', 'hour_of_day', 'day_of_week', 'merchant_category', 'location_distance'];
      case 'payment_patterns':
        return ['amount', 'payment_method', 'gateway', 'processing_time', 'retry_count'];
      case 'user_patterns':
        return ['session_duration', 'page_views', 'transaction_count', 'login_frequency'];
      default:
        return [];
    }
  }
  ...
  public async analyzeTransaction(metrics: AnalyticsMetrics): Promise<AnomalyScore[]> {
  ...
  private async updateUserProfile(metrics: AnalyticsMetrics): Promise<void> {
    ...
    profile.transactionHistory.averageAmount = 
      profile.transactionHistory.totalAmount / profile.transactionHistory.totalTransactions;
    ...
    if (metrics.location) {
      profile.transactionHistory.locations.add(JSON.stringify(metrics.location));
    }
  ...
  private async detectAmountAnomaly(metrics: AnalyticsMetrics): Promise<AnomalyScore | null> {
    const model = this.statisticalThresholds.get('transaction_amount');
    if (!model || !model.isCalibrated) {
      return null;
    }

    const zScore = Math.abs((metrics.amount - model.mean) / model.standardDeviation);
  ...
  private async detectFrequencyAnomaly(metrics: AnalyticsMetrics): Promise<AnomalyScore | null> {
    const windowKey = `${metrics.userId}_${metrics.type}`;
    const window = this.dataWindows.get(windowKey) || [];
  ...
  private async detectPatternAnomaly(metrics: AnalyticsMetrics): Promise<AnomalyScore | null> {
  ...
  private async detectVelocityAnomaly(metrics: AnalyticsMetrics): Promise<AnomalyScore | null> {
    ...
    const averageTimeSpan = timeSpans.reduce((a, b) => a + b, 0) / timeSpans.length;
    ...
    if (velocityRatio > 5.0) { // 5x normal velocity
      return {
  ...
  private async detectBehaviorAnomaly(metrics: AnalyticsMetrics): Promise<AnomalyScore | null> {
    const profile = this.userProfiles.get(metrics.userId);
    if (!profile) return null;
    ...
    const hourFrequency = profile.behaviorPatterns.preferredHours.get(hour) || 0;
    ...
    if (hourProbability < 0.05 && totalTransactions > 50) { // Less than 5% of transactions at this hour
      anomalies.push({
        type: 'unusual_time',
    ...
    if (metrics.location && profile.transactionHistory.locations.size > 5) {
      ...
      const isNewLocation = !profile.transactionHistory.locations.has(locationKey);
    ...
    if (metrics.merchantId && profile.transactionHistory.merchants.size > 10) {
      const isNewMerchant = !profile.transactionHistory.merchants.has(metrics.merchantId);
  ...
  private extractPatternFeatures(metrics: AnalyticsMetrics): Record<string, number> {
  ...
  private calculateLocationDistance(location: any): number {
    ...
    const a = Math.sin(dLat/2) * Math.sin(dLat/2) +
              Math.cos(this.toRadians(typicalLat)) * Math.cos(this.toRadians(location.latitude)) *
  ...
  private async calculateIsolationScore(features: Record<string, number>, model: IsolationForestModel): Promise<number> {
  ...
  private async processAnomalyScores(metrics: AnalyticsMetrics, scores: AnomalyScore[]): Promise<void> {
  ...
  private async generateAlert(metrics: AnalyticsMetrics, anomalyScore: AnomalyScore): Promise<void> {
    ...
    const alert: AnomalyAlert = {
      ...
      timestamp: Date.now(),
      status: 'active',
      investigationRequired: anomalyScore.severity === 'high' || anomalyScore.severity === 'critical',
      metadata: {
  ...
  private async updateUserRiskScore(userId: string, scores: AnomalyScore[]): Promise<void> {
    ...
    if (recentAnomalies.length > 0) {
      ...
      const avgScore = recentAnomalies.reduce((sum, s) => sum + s.score, 0) / recentAnomalies.length;
      
      // Weighted combination of current risk and new anomalies
      const newRiskScore = (profile.riskScore * 0.7) + (maxScore * 0.2) + (avgScore * 0.1);
  ...
  private async retrainIsolationForest(modelType: string, model: IsolationForestModel): Promise<void> {
    try {
      // Get training data from the last 30 days
      const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
      
      const trainingData = await this.mongodb.collection('analytics_transactions')
        .find({ timestamp: { $gte: thirtyDaysAgo } })
        .limit(10000) // Limit training data size
        .toArray();
      
      if (trainingData.length < 1000) {
        logger.warn(`Insufficient training data for ${modelType}: ${trainingData.length} samples`);
        return;
      }
      
      // Extract features from training data
      const features = trainingData.map(data => this.extractPatternFeatures(data));
      ...
      logger.info(`Retrained ${modelType} model with ${features.length} samples`);
  ...
  private async updateThreshold(modelType: string, threshold: StatisticalThreshold): Promise<void> {
    try {
      ...
      const values = recentData.map(d => this.extractValueForModel(modelType, d));
      ...
      logger.debug(`Updated ${modelType} threshold: mean=${mean.toFixed(2)}, std=${standardDeviation.toFixed(2)}`);
  ...
  private async getRecentDataForModel(modelType: string): Promise<any[]> {
    ...
    return await this.mongodb.collection('analytics_transactions')
  ...
  private async cleanupOldAlerts(): Promise<void> {
    try {
      ...
      const result = await this.mongodb.collection('anomaly_alerts')
  ...
  public async getActiveAlerts(userId?: string): Promise<AnomalyAlert[]> {
    try {
      ...
      const alerts = await this.mongodb.collection('anomaly_alerts')
        .find(query)
        .sort({ timestamp: -1 })
        .limit(100)
        .toArray();
  ...
  public async updateAlertStatus(alertId: string, status: string): Promise<boolean> {
    try {
      const alert = this.activeAlerts.get(alertId);
      if (alert) {
        alert.status = status;
        await this.mongodb.collection('anomaly_alerts')
