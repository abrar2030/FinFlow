...
# from models.cash_flow import (CashFlowMetrics, CashFlowOptimization,
#                               CashFlowPredictionRequest,
#                               CashFlowPredictionResponse, CashFlowScenario,
#                               LiquidityRisk)
...
##                         df["net_flow"].fillna(0), model="additive", period=30
#                    )
...
#                     features_df, future_date, feature_columns
                )

#                 future_features_scaled = self.scalers["standard"].transform(
#                     [future_features]
                )
...
#                 lr_pred = lr_model.predict(future_features_scaled)[0]
                ...
                # Time series models
#                 prophet_pred = await self._predict_with_prophet(
#                     features_df, future_date
                )
...
#                    ):
##                         features[col] = (
##                             last_row[col] if not pd.isna(last_row[col]) else 0
#                        )
...
##         self, features_df: pd.DataFrame, future_date: datetime
##     ) -> float:
...
#                     historical_data["inflow"].resample("M").sum().mean()
                ),
#                 average_monthly_outflow=float(
#                     historical_data["outflow"].resample("M").sum().mean()
                ),
#                 average_monthly_net_flow=float(
#                     historical_net_flow.resample("M").sum().mean()
                ),
...
#             negative_days = sum(
#                 1 for pred in predictions if pred["predicted_net_flow"] < 0
            )
#             if negative_days > len(predictions) * 0.2:
#                 emergency_fund = (
#                     abs(min(pred["predicted_net_flow"] for pred in predictions)) * 3
                )
#                 recommendations.append(
                    {
...
#                 action_items=[
#                     rec["recommendation"] for rec in recommendations[:3]
#                 ],  # Top 3
            )
...
##             cv = historical_data["net_flow"].std() / abs(
##                 historical_data["net_flow"].mean()
#            )
...
##             positive_ratio = sum(
##                 1 for flow in historical_data["net_flow"] if flow > 0
##             ) / len(historical_data)
...
#         return {"accuracy": 0.85, "mae": 150.0, "rmse": 200.0, "r2_score": 0.78}
...
##                 / (len(historical_data) * len(historical_data.columns))
#            )
#
#            # Consistency (no negative inflows, no positive outflows)
##             consistency_issues = sum(
##                 1 for inflow in historical_data["inflow"] if inflow < 0
#            )
##             consistency_issues += sum(
##                 1 for outflow in historical_data["outflow"] if outflow < 0
#            )
...
##             recency = max(
##                 0, 1 - days_since_last / 30
##             )  # Penalize if data is older than 30 days
#
...
#             cache_key = f"cash_flow_prediction:{user_id}"
            ...
            # Cache for 24 hours
#             await redis.setex(cache_key, 86400, response.json())
