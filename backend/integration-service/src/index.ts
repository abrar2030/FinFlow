  ...
  private getOAuthRoutes(): express.Router {
    ...
    router.get('/quickbooks/callback', async (req, res) => {
      try {
        ...
        res.redirect(`${config.frontend.baseUrl}/integrations/quickbooks/success?${new URLSearchParams(result)}`);
      } catch (error) {
        logger.error('QuickBooks OAuth callback error:', error);
        res.redirect(`${config.frontend.baseUrl}/integrations/quickbooks/error?error=${encodeURIComponent(error.message)}`);
      }
    });
    ...
    router.get('/xero/callback', async (req, res) => {
      try {
        ...
        res.redirect(`${config.frontend.baseUrl}/integrations/xero/success?${new URLSearchParams(result)}`);
      } catch (error) {
        logger.error('Xero OAuth callback error:', error);
        res.redirect(`${config.frontend.baseUrl}/integrations/xero/error?error=${encodeURIComponent(error.message)}`);
      }
    });

    // Plaid Link callback
    router.post('/plaid/link', tenantMiddleware, authMiddleware, async (req, res) => {
      try {
