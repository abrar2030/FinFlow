...
export class DataIsolationService extends EventEmitter {
    ...
    try {
      ...
      for (const row of result.rows) {
        try {
          ...
          logger.info(`Initialized database connection for tenant: ${row.slug}`);

        } catch (error) {
          logger.error(`Failed to initialize database for tenant ${row.slug}:`, error);
        }
      }
  ...
  async getData(
    ...
    try {
      // Log data access
      await this.logDataAccess(tenantId, options.userId, resource, null, 'read', options);
      ...
      switch (strategy) {
        case 'database':
          data = await this.getDataFromTenantDatabase(tenantId, resource, filters, options);
          break;
        case 'schema':
          data = await this.getDataFromTenantSchema(tenantId, resource, filters, options);
          break;
        case 'row':
        default:
          data = await this.getDataWithRowLevelSecurity(tenantId, resource, filters, options);
          break;
      }
  ...
  async createData(
    ...
    try {
      ...
      switch (strategy) {
        case 'database':
          result = await this.createDataInTenantDatabase(tenantId, resource, data, options);
          break;
        case 'schema':
          result = await this.createDataInTenantSchema(tenantId, resource, data, options);
          break;
        case 'row':
        default:
          result = await this.createDataWithRowLevelSecurity(tenantId, resource, data, options);
          break;
      }

      // Log data access
      await this.logDataAccess(tenantId, options.userId, resource, result.id, 'create', options);
  ...
  async updateData(
    ...
    try {
      ...
      switch (strategy) {
        case 'database':
          result = await this.updateDataInTenantDatabase(tenantId, resource, id, data, options);
          break;
        case 'schema':
          result = await this.updateDataInTenantSchema(tenantId, resource, id, data, options);
          break;
        case 'row':
        default:
          result = await this.updateDataWithRowLevelSecurity(tenantId, resource, id, data, options);
          break;
      }

      // Log data access
      await this.logDataAccess(tenantId, options.userId, resource, id, 'update', options);
  ...
  async deleteData(
    ...
    try {
      ...
      switch (strategy) {
        case 'database':
          await this.deleteDataFromTenantDatabase(tenantId, resource, id, options);
          break;
        case 'schema':
          await this.deleteDataFromTenantSchema(tenantId, resource, id, options);
          break;
        case 'row':
        default:
          await this.deleteDataWithRowLevelSecurity(tenantId, resource, id, options);
          break;
      }

      // Log data access
      await this.logDataAccess(tenantId, options.userId, resource, id, 'delete', options);
  ...
  async bulkOperations(
    ...
    try {
      ...
      for (const operation of operations) {
        try {
          ...
          switch (operation.type) {
            case 'create':
              result = await this.createData(tenantId, resource, operation.data, { ...options, client });
              break;
            case 'update':
              result = await this.updateData(tenantId, resource, operation.id!, operation.data, { ...options, client });
              break;
            case 'delete':
              await this.deleteData(tenantId, resource, operation.id!, { ...options, client });
      ...
      // Log bulk operation
      await this.logDataAccess(tenantId, options.userId, resource, null, 'bulk', {
        ...options,
  ...
  async exportData(
    ...
    try {
      ...
      const data = await this.getData(tenantId, resource, options.filters || {}, options);

      // Apply data masking for export
      const maskedData = await this.applyDataMasking(tenantId, resource, data, { ...options, export: true });
      ...
      switch (format.toLowerCase()) {
        ...
        case 'json':
          ...
          break;
        case 'zip':
          exportData = await this.exportToZip(tenantId, resource, maskedData, options);
          break;
        default:
      ...
      // Log export
      await this.logDataAccess(tenantId, options.userId, resource, null, 'export', {
        ...options,
        format,
      ...
      await this.storeBackupMetadata(tenantId, resource, format, exportData, options);
  ...
  private async getTenantIsolationStrategy(tenantId: string): Promise<string> {
    try {
      ...
      const strategy = result.rows.length > 0 ? result.rows[0].isolation_type : 'row';
  ...
  private async getDataFromTenantDatabase(
    ...
    try {
      const { query, values } = this.buildSelectQuery(resource, filters, options);
  ...
  private async getDataFromTenantSchema(
    ...
    try {
      ...
      const { query, values } = this.buildSelectQuery(resource, filters, options, schemaName);
      const result = await client.query(query, values);
      return result.rows;
  ...
  private async getDataWithRowLevelSecurity(
    ...
    try {
      ...
      const { query, values } = this.buildSelectQuery(resource, tenantFilters, options);
      const result = await client.query(query, values);
      return result.rows;
  ...
  private async createDataInTenantSchema(
    ...
    try {
      const schemaName = `tenant_${tenantId.replace(/-/g, '_')}`;
      const { query, values } = this.buildInsertQuery(resource, data, schemaName);
      const result = await client.query(query, values);
      return result.rows[0];
  ...
  private async updateDataInTenantSchema(
    ...
    try {
      const schemaName = `tenant_${tenantId.replace(/-/g, '_')}`;
      const { query, values } = this.buildUpdateQuery(resource, id, data, schemaName);
      const result = await client.query(query, values);
  ...
  private async updateDataWithRowLevelSecurity(
    ...
    try {
      const { query, values } = this.buildUpdateQuery(resource, id, data, null, tenantId);
      const result = await client.query(query, values);
  ...
  private async validateData(resource: string, data: any, isUpdate: boolean = false): Promise<void> {
        ...
        switch (rule.type) {
          ...
          case 'number':
            ...
            if (rule.min !== undefined && value < rule.min) {
              throw new Error(`Field ${rule.field} must be at least ${rule.min}`);
            }
            if (rule.max !== undefined && value > rule.max) {
              throw new Error(`Field ${rule.field} must be at most ${rule.max}`);
            }
            break;
  ...
  private async encryptSensitiveData(tenantId: string, resource: string, data: any): Promise<any> {
    ...
    for (const field of fieldsToEncrypt) {
      if (encryptedData[field]) {
        ...
        encryptedData[field] = `encrypted:${Buffer.from(encryptedData[field]).toString('base64')}`;
      }
    }
  ...
  private async exportToCSV(data: any[], options: DataExportOptions): Promise<string> {
  ...
  private async cacheData(tenantId: string, resource: string, filters: any, data: any[]): Promise<void> {
    try {
  ...
  private async clearDataCache(tenantId: string, resource: string): Promise<void> {
    try {
