...
class MultiTenantApp {
  ...
  private initializeRoutes(): void {
    ...
    this.app.use('/api/admin/tenants', 
      authMiddleware, 
    ...
    this.app.use('/api/admin/isolation', 
      authMiddleware, 
      this.requireAdminRole.bind(this),
  ...
  private requireAdminRole(req: express.Request, res: express.Response, next: express.NextFunction): void {
  ...
  private getTenantSpecificRoutes(): express.Router {
    ...
    router.put('/settings', async (req, res) => {
      ...
      const updatedSettings = await this.tenantService.updateTenantSettings(tenantId, req.body);
  ...
  private getDataAccessRoutes(): express.Router {
    ...
    router.get('/:resource', async (req, res) => {
      ...
      const data = await this.dataIsolationService.getData(tenantId, resource, filters);
      res.json(data);
    });
    ...
    router.post('/:resource', async (req, res) => {
      ...
      const result = await this.dataIsolationService.createData(tenantId, resource, data);
    ...
    router.put('/:resource/:id', async (req, res) => {
      ...
      const result = await this.dataIsolationService.updateData(tenantId, resource, id, data);
      res.json(result);
    });
    ...
    router.post('/:resource/bulk', async (req, res) => {
      ...
      const results = await this.dataIsolationService.bulkOperations(tenantId, resource, operations);
      res.json(results);
    });
    ...
    router.get('/:resource/export', async (req, res) => {
      ...
      const exportData = await this.dataIsolationService.exportData(tenantId, resource, format);
      
      if (format === 'csv') {
        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', `attachment; filename="${resource}-${tenantId}.csv"`);
      } else {
        res.setHeader('Content-Type', 'application/json');
        res.setHeader('Content-Disposition', `attachment; filename="${resource}-${tenantId}.json"`);
      }
