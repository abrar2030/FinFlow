...
import { TransactionCreateInput, TransactionUpdateInput } from '../types/transaction.types';
...
class TransactionController {
  ...
  async getTransactionById(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (transaction.userId !== userId) {
        res.status(403).json({ message: 'Forbidden: You do not have access to this transaction' });
        return;
      }
  ...
  async getTransactionsByUserId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user.sub; // Get user ID from JWT token
  ...
  async updateTransaction(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Update transaction
      const updatedTransaction = await transactionService.update(id, transactionInput);
  ...
  async getTransactionsByDateRange(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user.sub; // Get user ID from JWT token
