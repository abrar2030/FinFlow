...
class InvoiceController {
  ...
  async getInvoiceById(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (invoice.userId !== userId) {
        res.status(403).json({ message: 'Forbidden: You do not have access to this invoice' });
        return;
      }
  ...
  async getInvoicesByUserId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user.sub; // Get user ID from JWT token
