...
class ForecastController {
  ...
  async generateForecast(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Generate forecast
      const forecast = await forecastService.generateForecast(forecastRequest.userId, forecastRequest.months);
  ...
  async getForecastsByUserId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user.sub; // Get user ID from JWT token
