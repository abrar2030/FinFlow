...
class ComplianceService:
    ...
    def create_psd2_consent(self, request: PSD2AuthRequest) -> PSD2AuthResponse:
        ...
        expires_at = datetime.now() + timedelta(
            days=min(request.duration_days, self.config["psd2"]["max_consent_days"])
        )
        ...
        # Log for audit
        logger.info(
            f"PSD2 consent {consent_id} created for user {request.user_id}, TPP: {request.third_party_provider}"
        )
    ...
    def revoke_psd2_consent(self, consent_id: str) -> bool:
        ...
        if consent_id in self._psd2_consents:
            ...
            # Log for audit
            logger.info(
                f"PSD2 consent {consent_id} revoked for user {consent['user_id']}"
            )
    ...
    def validate_psd2_consent(self, consent_id: str, scope: str) -> bool:
        ...
        expires_at = (
            datetime.fromisoformat(consent["expires_at"])
            if isinstance(consent["expires_at"], str)
    ...
    def screen_entity(self, request: AMLScreeningRequest) -> AMLScreeningResponse:
        ...
        # Log for audit
        logger.info(
            f"AML screening {screening_id} for {request.entity_type} {request.entity_name}"
        )
    ...
    def schedule_rescreening(self, entity_id: str, entity_type: str) -> str:
        ...
        next_screening_date = datetime.now() + timedelta(
            days=self.config["aml"]["rescreening_interval_days"]
        )
