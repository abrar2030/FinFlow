...
class ComplianceService:
    ...
    def get_gdpr_request_status(self, request_id: str) -> Optional[GDPRDataResponse]:
        ...
        if request_id in self._data_requests:
            ...
            # FIX: The data dictionary contains datetime objects which Pydantic can handle,
            # but it's safer to ensure it's loaded correctly.
    ...
    def validate_psd2_consent(self, consent_id: str, scope: str) -> bool:
        ...
        if consent.expires_at < datetime.now():
            # In production, this would update the status to EXPIRED
            return False
...
async def create_gdpr_request(
    ...
    """
    try:
        # NOTE: The original code included background_tasks but didn't use it.
        # I will keep it for future async processing if needed, but the mock is synchronous.
...
async def revoke_psd2_consent(consent_id: str):
    ...
    # FIX: Explicitly return None for 204 No Content
    return None
