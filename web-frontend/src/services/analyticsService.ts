...
export const createTransaction = async (transactionData: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<Transaction> => {
...
export const updateTransaction = async (id: string, transactionData: Partial<Transaction>): Promise<Transaction> => {
  const response = await api.put(`/analytics/transactions/${id}`, transactionData);
  return response.data;
};
...
export const getForecast = async (startDate: string, endDate: string, type: 'revenue' | 'expense' | 'cashflow'): Promise<any> => {
